...
template.helper('images_site',function($img){
	 ;
	if($img==null) return ROOT+"img/no_image.jpg";
	if($img.match(/http:/) || $img.match(/https:/)){
		return $img;
	}else{
		return IMAGES_SITE+$img;
	}
});

template.helper('parseInt',function($v){
	 return parseInt($v);
});

template.helper('math',function(v,str){
	return eval(v+str);
})

template.helper('timeago',function(dateline){
	//天数
	var time=new Date().getTime()/1000-dateline;
	var day=time/86400;
	...
	//秒
	var second=(time-86400*day-3600*hour-minute*60);
