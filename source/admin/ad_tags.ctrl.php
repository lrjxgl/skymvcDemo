<?php
	...
	class ad_tagsControl extends skymvc{
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=$this->ad_tags->select($option,$rscount);
			if($data){
				foreach($data as $k=>$v){
					$child=$this->ad_tags->select(array("where"=>"  pid=".$v['tag_id'],"order"=>"orderindex asc"));
					$v['child']=$child;
					$data[$k]=$v;
				}
			}
			...
			$this->smarty->assign(
				array(
					...
					"rscount"=>$rscount,
					"url"=>$url,
					"pid_list"=>$this->ad_tags->tagList(0,0)
				)
			);
		...
		public function onShow(){
			...
			if($tag_id){
				$data=$this->ad_tags->selectRow(array("where"=>"tag_id={$tag_id}"));
		...
		public function onSave(){
			...
			$row=$this->ad_tags->selectRow(array("where"=>"tagno='".$data['tagno']."' "));
			if($data['tagno'] && $row['tag_id'] && $row['tag_id']!=$tag_id){
				$this->goall("唯一代号已存在，请换一个",1);
			}
			if($tag_id){
				$this->ad_tags->update($data,"tag_id='$tag_id'");
			}else{
				$data["dateline"]=time();
				
				$this->ad_tags->insert($data);
			}
			$this->goall("保存成功");
		}
		...
		public function onStatus(){
			...
			$this->ad_tags->update(array("status"=>$status),"tag_id=$tag_id");
			exit(json_encode(array("error"=>0,"message"=>"状态修改成功")));
		}
		
		public function onDelete(){
			$tag_id=get_post('tag_id',"i");
			$this->ad_tags->delete("tag_id={$tag_id}");
		...
		public function onChild(){
			$tag_id=get('tag_id','i');
			if($tag_id){
				$tag=$this->ad_tags->selectRow(array("where"=>" tag_id=".$tag_id." "));
				$data=$this->ad_tags->select(array("where"=>"pid=".$tag_id));
