...
while($rs=mysqli_fetch_array($res,MYSQLI_ASSOC)){
	...
	if($i==0){
		...
		if($rs['Field']=='dateline'){
			$form.="  {if \$data}<tr><td>".($rs['Comment']?$f[0]:$rs['Field'])."：</td><td>{\${$data}.{$rs['Field']}|date:Y-m-d H:m}</td></tr>{/if}\r\n";
		}else{
			$form.="  <tr><td>".($rs['Comment']?$f[0]:$rs['Field'])."：</td><td><input type=\"text\" name=\"{$rs['Field']}\" id=\"{$rs['Field']}\" value=\"{\${$data}.{$rs['Field']}}\" ></td></tr>\r\n";
		}
	}
}
$form.="<tr><td></td><td><input type=\"submit\" value=\"保存\" class=\"btn btn-success\"></td></tr>";
$form.=" </table>\r\n";
