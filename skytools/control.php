	...
	*/
	...
	class '.$c.'Control extends skymvc{
		...
		public function onDelete(){
			$'.$p_key.'=get_post(\''.$p_key.'\',"i");
			M("'.$model.'")->update(array("status"=>99),"'.$p_key.'=$'.$p_key.'");
			$this->goall("删除成功",0);
		}
		';
		$str.='
...
if(!file_exists(ROOT_PATH."source/{$dir}/{$c}.ctrl.php") or $copy){
	...
	if($istpl){
		...
		$content=str_replace("[操作]",'<a href="'.$dir.'.php?m='.$c.'&a=add&'.$p_key.'={$c.'.$p_key.'}">编辑</a> <a href="'.$dir.'.php?m='.$c.'&a=show&'.$p_key.'={$c.'.$p_key.'}">查看</a> <a href="javascript:;" class="delete" url="'.$dir.'.php?m='.$c.'&a=delete&'.$p_key.'={$c.'.$p_key.'}">删除</a>',$content);
