...
<form method="get" action="modulecontrol.php">
  <p>文件名称：
    ...
    模块：<input type="text" name="module" value="<?php echo $_GET['module'];?>" />
     ...
     <select name="tpl">
     	<?php
			$tpls=array("index.tpl","admin.tpl","user.tpl","shopadmin.tpl","wap_index.tpl");
        	foreach($tpls as $v){
				if($v==$_GET['tpl']){
	...
	*/
	...
	class '.$c.'Control extends skymvc{
		...
		public function onStatus(){
			...
			M("'.$model.'")->update(array("bstatus"=>$bstatus),"'.$p_key.'=$'.$p_key.'");
			$this->goall("状态修改成功");
		}
		';
		...
		public function onDelete(){
			$'.$p_key.'=get_post(\''.$p_key.'\',"i");
			M("'.$model.'")->update(array("bstatus"=>11),"'.$p_key.'=$'.$p_key.'");
			$this->goAll("删除成功");
...
if(!file_exists(ROOT_PATH."module/{$module}/source/{$dir}/{$c}.ctrl.php") or $copy){
	...
	if($istpl){
		...
		$content=str_replace("[操作]",'<a href="/module.php?m='.$c.'&a=add&'.$p_key.'={$c.'.$p_key.'}">编辑</a> <a href="/module.php?m='.$c.'&a=show&'.$p_key.'={$c.'.$p_key.'}">查看</a> <a href="javascript:;" class="delete" url="/module.php?m='.$c.'&a=delete&'.$p_key.'={$c.'.$p_key.'}">删除</a>',$content);
		...
		if($add){
			...
			preg_match("/<pre.*>(.*)<\/pre>/iUs",$con,$d);
			$content=str_replace(array("[nav]","[tpl]"),array("{include file='{$tpldir}/nav.html'}",html($d[1])),$tplcontent);
			$content=str_replace("[action]",'/module.php?m='.$c."&a=save",$content);
			$content=str_replace("&quot;",'"',$content);
