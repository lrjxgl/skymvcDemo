...
?>
	...
	<%for(var i=0;i<list.length;i++){%> 
		<%var $value=list[i];%>	  	
        		<li class="mui-table-view-cell">
        			<a href="/index.php?m=article&a=show&id=<%=$value.id%>" class="mui-navigate-right">
        			<%if($value.imgurl){%>
        			<img class="mui-media-object mui-pull-left" src="<%=$value.imgurl%>.100x100.jpg">
                    <div class="mui-media-body">
                       <%=$value.title%>
                        <p class='mui-ellipsis'><%=$value.description%></p>
                    </div>
                    <%}else{%>}
        				<%=$value.title%>
        			<%}%>
	...
	<script src="<?php echo $this->_var['skins']; ?>article/list.js"></script>
