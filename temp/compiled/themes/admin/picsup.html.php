...
<div class="picsup-box">
            ...
            <div class="row" style=" width:90%; margin: 0 auto; margin-bottom:20px;">
                	...
                	<div class="btn-upload" style="background:#333; text-align:center; line-height:30px; color:#fff;"><i class="iconfont icon-upload"></i> 上传图集 <input type="file" multiple  id="picsup" class="btn-upload-file"></div>
                 
                   
              </div>
            <div id="pics" class="row">             
            	<div id="webuploader" url="/index.php?m=imgs&tablename=<?php echo $this->_var['tablename']; ?>&object_id=<?php echo $this->_var['data']['id']; ?>" class="webuploader">
                    <div class="ks-list" >
                                <div class="item">
                                </div>
...
function picsupload(i)
{
		...
		oXHR.open('POST',"/index.php?m=upload&a=img&dir=forum&id=<?php echo $this->_var['data']['id']; ?>");
		oXHR.send(vFD);
...
function picsupsuccess(e){
			...
			$.post("/index.php?m=imgs&a=save&tablename=<?php echo $this->_var['tablename']; ?>&object_id=<?php echo $this->_var['data']['id']; ?>",{imgurl:data.data.imgurl},function(data){
				modImgsList("webuploader",1)
			},"json");
	}
